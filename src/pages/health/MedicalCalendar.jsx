...
export function MedicalAppointmentWizard({ close, draft, onDraftChange, patients, professionals, services, onNewPatient, onSaved }) {
...
export default function MedicalCalendar({ operator = false }) {
  ...
  return <Shell action={<div className="flex gap-2"><Button icon="today" onClick={() => { setCursor(new Date(now.getFullYear(), now.getMonth(), 1)); setSelected(today); }} variant="secondary">Hoy</Button>{canSchedule ? <Button disabled={selected < today} icon="event_available" onClick={openSchedule}>Agendar cita</Button> : null}</div>} subtitle="Agenda médica por servicio, profesional, estado y expediente clínico." title="Agenda médica">
